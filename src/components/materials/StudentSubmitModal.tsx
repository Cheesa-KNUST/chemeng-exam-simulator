...
import { X, Upload, Loader2, Plus, Tag, Info } from "lucide-react";
...
export default function StudentSubmitModal({ onClose, onSuccess }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="mx-5 mt-5 flex gap-3 p-3.5 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50">
          <Info
            size={15}
            className="text-amber-500 dark:text-amber-400 shrink-0 mt-0.5"
          />
          <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
            Submitted materials are reviewed by the CHEESA Tratech Committee
            before they appear in the library. You&apos;ll see them listed once
            approved.
          </p>
        </div>
        ...
        <div className="px-5 py-5 space-y-4">
          ...
          <textarea
            placeholder="Description — what is this material about? (optional)"
          ...
          <div className="space-y-2">
            <div className="flex gap-2">
              <input
                type="text"
                placeholder="Add tag (e.g. fluid mechanics)"
                value={tagInput}
